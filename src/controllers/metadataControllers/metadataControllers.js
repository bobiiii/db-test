...
const addMetadata = asyncHandler(async (req, res, next) => {
  ...
  if (openGraphImage) {
    uploadtoS3 = await uploadImageToDrive(openGraphImage);
  } else {
...
const updateMetadata = asyncHandler(async (req, res, next) => {
  ...
  if (openGraphImage) {
    metadata.ogImageId = await updateImageOnDrive(metadata.ogImageId, openGraphImage);
  }
...
const deleteMetadata = asyncHandler(async (req, res, next) => {
  ...
  await deleteImage(metadata.ogImageId);
  if (!metadata) {
    return next(new ErrorHandler('No metadata found  ', 400));
  }
